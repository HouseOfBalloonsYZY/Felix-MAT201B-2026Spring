...
struct MyApp : public App
{
    ...
    std::vector<int> toLoveNeighbour;

    // XXX dis is nested function, not good
    void randomlyFallInLove()
    {
        toLoveNeighbour.clear();
        toLoveNeighbour.resize(agent.size());
        for (int i = 0; i < (int)toLoveNeighbour.size(); i++)
        {
            toLoveNeighbour[i] = rand() % (agent.size() - 1);
            while (toLoveNeighbour[i] == i)
            {
                toLoveNeighbour[i] = rand() % (agent.size() - 1);
            }
        }
    ...
    void onAnimate(double dt) override
    {
        ...
        for (int i = 0; i < agent.size(); i++)
        {
            ...
            agent[i].faceToward(agent[toLoveNeighbour[i]].pos(), turnSpeed);
        }
